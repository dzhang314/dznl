...
namespace dznl {
    ...
    public: // =========================================================== BIG 3
        explicit MPFRVector(const MPFRVector &rhs)
                : size_(rhs.size_), entries(size_), precision(rhs.precision) {
    ...
    public: // =================================================================
        ...
        // incompatible sizes?
        void set_matrix_vector_multiply(const MPFRMatrix &mat,
                                        const MPFRVector &vec,
                                        mpfr_rnd_t rnd) {
            for (std::size_t i = 0, k = 0; i < size_; ++i) {
                mpfr_mul(entries[i], mat.data()[k], vec[0], rnd);
                ++k;
                for (std::size_t j = 1; j < size_; ++j, ++k) {
                    mpfr_fma(entries[i], mat.data()[k], vec[j], entries[i],
                             rnd);
                }
            }
        }
