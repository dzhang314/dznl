...
namespace dznl {
    ...
    public: // ===================================================== CONSTRUCTOR
        ...
        explicit MPFRVector(std::size_t size, mpfr_prec_t prec) :
                size(size), entries(size), precision(prec) {
            for (mpfr_t &x : entries) { mpfr_init2(x, precision); }
        }

    public: // =========================================================== BIG 3

        MPFRVector(const MPFRVector &rhs) :
                size(rhs.size), entries(size), precision(rhs.precision) {
        ...
        MPFRVector &operator=(MPFRVector rhs) {
    ...
    public: // =================================================================
        ...
        // Add an optional assertion to check that size == rhs.size.
