...
- **Zero dependencies** &mdash; ***not even the C++ standard library!*** **`dznl`** is great for environments like WebAssembly where a standard library implementation may not be available.

- **Zero dynamic allocation** &mdash; **`dznl`** does not contain a single call to `malloc` or `operator new` (except in unit tests). This makes **`dznl`** suitable for resource-constrained environments where memory must be carefully managed.

- **No recursion or exceptions** &mdash; **`dznl`** is designed to have completely predictable static resource allocation with no possibility of stack overflow or unwinding.
