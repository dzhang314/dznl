...
static constexpr double my_rosenbrock_objective(my_accessor_t &x) {
...
static constexpr bool my_null_constraint(my_accessor_t &) { return true; }
...
TEST_CASE("NelderMeadOptimizer mut_index_t") {
    ...
    Optimizer optimizer(
        ...
        mut_null_constraint,
        mut_accessor_t(initial_point.data(), test_only_t()),
        mut_index_t(2, test_only_t()),
        0.5,
        mut_accessor_t(workspace.data(), test_only_t())
    );
    REQUIRE(workspace[0] == 2.0);
    REQUIRE(workspace[1] == 4.0);
    REQUIRE(workspace[2] == 1.0);
